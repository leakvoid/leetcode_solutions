class Solution:
    def interleave_rec(self, s1, s2, s3, s1_i, s2_i):
        while s1_i < len(s1) and s2_i < len(s2):
            ...
            if s1[s1_i] == s3[s3_i]:
                if s1[s1_i] == s2[s2_i]:
                    shift1 = 1
                    while s1_i + shift1 < len(s1):
                        if s1[s1_i + shift1] == s3[s3_i + shift1] and s1[s1_i] == s1[s1_i + shift1]:
                            shift1 += 1
                        else:
                            break
                    mismatch_shift1 = shift1
                    
                    shift2 = 1
                    while s2_i + shift2 < len(s2):
                        if s2[s2_i + shift2] == s3[s3_i + shift2] and s2[s2_i] == s2[s2_i + shift2]:
                            shift2 += 1
                        else:
                            break
                    mismatch_shift2 = shift2

                    #shift = min(mismatch_shift1, mismatch_shift2)
                    return self.interleave_rec(s1, s2, s3, s1_i + mismatch_shift1, s2_i) or self.interleave_rec(s1, s2, s3, s1_i, s2_i + mismatch_shift2)
                else:
                    s1_i += 1
    ...
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        return self.interleave_rec(s1, s2, s3, 0, 0)
